...
## Сессия 2026-04-26 / audit-fixes-batch-1

7 правок по аудиту (_AUDIT.md) собраны в 6 атомарных коммитов:

1. **fix: show LoadingSplash only on first visit** (C-1)
   - localStorage-флаг `anhel_first_visit_done`. Splash больше не
     перекрывает контент при каждой client-side навигации.
   - .gitignore дополнен: _AUDIT.md, _audit-screenshots/ — артефакты
     локального аудита, не пушатся.

2. **fix: renumber sections on product pages and ITP modules** (C-2)
   - Tag-поля в 5 насосных + ВПУ перенумерованы:
     03 ПРИМЕНЕНИЕ → 04, 04 БРЕНДЫ → 05, 05 ПРЕИМУЩЕСТВА → 06, 06 ГАЛЕРЕЯ → 07.
   - TechSpecsGrid mono: «02 · Технические характеристики» → uppercase
     «02 · ТЕХНИЧЕСКИЕ ХАРАКТЕРИСТИКИ».
   - На [slug]/page.tsx ИТП-модулей tag overrides:
     QuizSection «05 · ОПРОСНЫЙ ЛИСТ», ProductCtaFooter «07 · ЗАПРОС КП».

3. **fix: remove duplicate ANHEL® in page titles** (C-3)
   - layout.tsx template `"%s · ANHEL®"` → `"%s"`. metaTitle уже содержит
     «ANHEL» в формулировке, template дописывал второй.
   - openGraph.title в 6 product-pages: убран хвост `· ANHEL®`.

4. **feat: add breadcrumbs to project detail pages** (C-5)
   - На /projects/[slug] добавлена полная цепочка
     «Главная > Объекты > [название]» в стилистике /products.
   - Back-link «← Все объекты» сохранён ниже.

5. **fix: correct H1 spacing on home page** (V-2 + /projects copy)
   - HeroTitle: aria-label с корректной строкой
     «Системы, которые держат здание живым.» — для screen reader / SEO.
   - /projects описание: «Setl Group и ПИК» → «крупных девелоперов
     Москвы и Санкт-Петербурга».

6. **refactor: remove QuizSection from ITP parent for consistency** (V-1)
   - На /products/heating-unit удалены QuizSection и CasesCarousel
     (оба placeholder без контента).
   - Documents и footerCta получили локальные tag-overrides
     «06 · ДОКУМЕНТАЦИЯ» и «07 · ЗАПРОС КП» — нумерация без разрывов.
   - Дочерние [slug]-страницы модулей не задеты.

**Build:** tsc clean, npm run build clean — 43 страницы.

**Не вошло в этот batch (по запросу):**
- C-4 «Опросный лист — СКОРО» на главной — оставлено
- V-5 large hero PNG — новых картинок ещё нет
- Hero заголовок — оставлен текущий
- Slabye формулировки на product-страницах
