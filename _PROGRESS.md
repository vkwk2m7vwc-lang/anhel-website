...
#ЗадачаВеткаСтатус1Объекты (портфолио) — насосные + водоподготовка`feat/projects-portfolio`✅ merged2Фото производства MFMC + ретушь`feat/production-photos`⛔ блокер: ручная ретушь3Сертификаты в существующий блок`feat/certificates`✅ merged (через fillable-forms)43 fillable PDF опросника`feat/fillable-forms`✅ merged5Руководство по эксплуатации (локально)—✅ saved локально, требует мелкой правки
