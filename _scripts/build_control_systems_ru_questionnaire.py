...
Why this exists
---------------
The original RU master at public/docs/control-systems/oprosnyi-list.pdf
was the un-rebranded MFMC "Omega Control" original (1.9 MB, MFMC
requisites and brand names in field labels). EN and TR versions had
already been rebuilt in the ANHEL wave-3 layout (≈36 KB each, ReportLab,
two A4 pages), so the RU file was the lone holdout before v1.20.

This script reuses the wave-3 generator from history commit 03254aa
with three local edits:

  1. ROOT path read from the ANHEL_ROOT env var (falls back to the
     project default), so it runs on any developer machine.
  2. A "ru" CHROME entry added next to "en" / "tr" — ANHEL wordmark,
     Profit LLC contact strip, Russian chrome labels (ОПРОСНЫЙ ЛИСТ,
     Шаг, Страница из, etc.).
  3. The build loop is narrowed to (control_systems × ru). The RU
     output keeps the canonical filename `oprosnyi-list.pdf` (no locale
     suffix) so existing links in /documents and product pages keep
     working unchanged.

Result: a 2-page ANHEL-branded RU questionnaire (~44 KB) that matches
the existing EN/TR siblings in layout and visual language. No MFMC
references — the field semantics are identical to the source quiz.json
(content rule per project: 1:1 with MFMC, just under ANHEL brand).
...
Requirements: reportlab (install with /usr/bin/python3 -m pip install
--user reportlab). On macOS this runs on the bundled Python 3.9 because
Homebrew Python 3.14 currently has a broken pyexpat dylib.
"""
...
import os; ROOT = Path(os.environ.get("ANHEL_ROOT","/Users/alexeyanurin/Projects/anhel-website"))
...
CHROME = {
    ...
    "ru": {
        "doc_kind": "ОПРОСНЫЙ ЛИСТ",
        "instructions_title": "Как заполнить форму",
        "instructions": (
            "Заполните форму от руки или в PDF-редакторе. Отметьте "
            "соответствующие поля (☐ → ■). При отсутствии данных оставьте "
            "строку пустой или укажите «уточняется». Направьте заполненный "
            "опросный лист на sales@anhelspb.com — инженерный отдел "
            "ответит подбором в течение одного рабочего дня."
        ),
        "company_strip": (
            "ООО «Профит» · бренд ANHEL® · Санкт-Петербург, Россия · "
            "anhelspb.com · sales@anhelspb.com · +7 (812) 416-45-00"
        ),
        "page": "Страница",
        "of": "из",
        ...
        "no": "Нет",
        "appendix_note": (
            "При ответе приложите чертежи проекта, анализы воды или "
            "технические задания отдельными файлами."
        ),
        "doc_id_prefix": "Номер документа",
        "date_prefix": "Дата",
        "step_label": "Шаг",
    },
    "tr": {
...
def build_all():
    ...
    for kind in ("control_systems",):
        for locale in ("ru",):
            dirs = QUIZ_TO_DIRS[kind]
            # Generate once into the first dir, then copy to siblings.
            primary = DOCS / dirs[0] / ("oprosnyi-list.pdf" if locale == "ru" else f"oprosnyi-list-{locale}.pdf")
            build_one(kind, locale, primary)
            size = primary.stat().st_size
            written.append((kind, locale, primary, size))
            for extra in dirs[1:]:
                target = DOCS / extra / ("oprosnyi-list.pdf" if locale == "ru" else f"oprosnyi-list-{locale}.pdf")
