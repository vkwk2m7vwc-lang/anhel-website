 ...
 *  Optional `id` прокидывается на <p> чтобы родительский
 *  `role="radiogroup"` / `role="group"` wrapper мог указать на
 ...
 * с id на заголовке группы — screen reader зачитывает название
 * группы перед списком вариантов, контекст не теряется.
 */
export function FieldRadioGroup({
  ...
  label: string;
  idPrefix: string;
  /** "radio" — взаимоисключающие, "checkbox" — multi-select. */
